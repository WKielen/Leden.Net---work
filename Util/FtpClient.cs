...
namespace Util.Comms
{
	...
	public class FtpClient
	{
		...
		public delegate void PrintLineDelegate(string printLine);
		public PrintLineDelegate printLine;
		...
		public FtpClient(string server, string username, string password, PrintLineDelegate printLine)
		{
			this.server = server;
		...
		public void Login()
		{
			...
			printLine("Opening connection to " + this.server);
			...
			printLine( "Connected to " + this.server );
		...
		public void Close()
		{
			printLine("Closing connection to " + this.server);
		...
		public void Download(string remFileName,string locFileName,Boolean resume)
		{
			...
			printLine("Downloading file " + remFileName + " from " + server + "/" + remotePath);
			...
			if ( resume )
			{
				...
				if ( offset > 0 )
				{
					...
					if ( this.resultCode != 350 )
					{
						...
						offset = 0;
						printLine("Resuming not supported:" + result.Substring(4));
					}
					else
					{
						printLine("Resuming at offset " + offset);
		...
		public void Upload(string fileName, bool resume)
		{
			...
			{
				// different file size
				printLine("Overwriting " + fileName);
				offset = 0;
			}
			else if ( resume && input.Length == offset )
			{
				// file done
				input.Close();
				printLine("Skipping completed " + fileName + " - turn resume off to not detect.");
				return;
			}
			...
			if ( offset > 0 )
			{
				...
				if ( this.resultCode != 350 )
				{
					printLine("Resuming not supported");
					offset = 0;
				}
			}
			...
			printLine( "Uploading file " + fileName + " to " + remotePath);
		...
		public void DeleteFile(string fileName)
		{
			...
			printLine( "Deleted file " + fileName);
		}
		...
		public void RenameFile(string oldFileName,string newFileName, bool overwrite)
		{
			...
			printLine( "Renamed file " + oldFileName + " to " + newFileName);
		}
		...
		public void MakeDir(string dirName)
		{
			...
			printLine( "Created directory " + dirName);
		}
		...
		public void RemoveDir(string dirName)
		{
			...
			printLine( "Removed directory " + dirName);
		}
		...
		public void ChangeDir(string dirName)
		{
			...
			printLine( "Current directory is " + this.remotePath);
		}
		...
		{
			if ( this.verboseDebugging ) printLine(command);
...
/*
				...
				foreach ( MethodInfo method in methods )
				{
					...
					printLine("private delegate " + method.ReturnType.Name + " " + method.Name + "Callback(" + param.TrimEnd(',') + ");");

					printLine("public System.IAsyncResult Begin" + method.Name + "( " + param + " System.AsyncCallback callback )");
					printLine("{");
					printLine("" + method.Name + "Callback ftpCallback = new " + method.Name + "Callback(" + values + " this." + method.Name + ");");
					printLine("return ftpCallback.BeginInvoke(callback, null);");
					printLine("}");
					printLine("public void End" + method.Name + "(System.IAsyncResult asyncResult)");
					printLine("{");
					printLine(method.Name + "Callback fc = (" + method.Name + "Callback) ((AsyncResult)asyncResult).AsyncDelegate;");
					printLine("fc.EndInvoke(asyncResult);");
					printLine("}");
					//printLine(method);
				}
*/
